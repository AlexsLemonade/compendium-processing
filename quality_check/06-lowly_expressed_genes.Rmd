...
# save the expression matrix as an RDS file -- this will be compressed enough
# to track with regular ol' git
saveRDS(exprs_mat, file.path("data", "refinebio_test_compendium_exprs_mat.RDS"))
```
...
```{r}
example_ecdf_plot <- data.frame(x = seq_mat[, 34]) %>%
...
```{r}
example_mode <- calculate_mode(seq_mat[, 34])
...
data.frame(x = seq_mat[, another_random_index]) %>%
  ...
  ggplot2::labs(y = "ecdf(x)", x = "expression value (x)") +
  ggplot2::theme_bw() +
  ggplot2::geom_vline(xintercept = calculate_mode(seq_mat[, another_random_index]),
                      lty = "dotted")
```
...
```{r}
seq_col_modes <- apply(seq_mat, 2, calculate_mode)
```

Find the indices of each of the "used-to-be-zero" values

```{r}
zero_gene_list <- 
  lapply(1:ncol(seq_mat),
         function(x) rownames(seq_mat)[which(seq_mat[, x] == seq_col_modes[x])])
```
...
(Presumably, genes missing in all RNA-seq samples in this test case have
been filtered out or genes missing in ~250 RNA-seq samples or more).
