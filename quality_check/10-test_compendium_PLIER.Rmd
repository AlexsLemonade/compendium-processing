...
both_tech_plier_list <- 
  lapply(seeds,
         function(x) PLIERWrapper(exprs_mat = exprs_mat,
...
```{r}
both_tech_pathway_coverage <- reshape2::melt(lapply(both_tech_plier_list, 
                                                    GetPathwayCoverage))
```
...
microarray_plier_list <-
  lapply(seeds, 
         function(x) PLIERWrapper(exprs_mat = microarray_mat,
                                  common_genes = common_genes,
                                  pathway_mat = combine_path_mat,
                                  seed = x)
  )
microarray_pathway_coverage <- reshape2::melt(lapply(microarray_plier_list,
                                                     GetPathwayCoverage))
```
...
rnaseq_plier_list <- 
  lapply(seeds, 
         function(x) PLIERWrapper(exprs_mat = rnaseq_mat,
                                  common_genes = common_genes,
                                  pathway_mat = combine_path_mat,
                                  seed = x )
         )
rnaseq_pathway_coverage <- reshape2::melt(lapply(rnaseq_plier_list, 
                                                 GetPathwayCoverage))
```
...
readr::write_tsv(coverage_df, 
                 file.path(result_dir,
                           "test_compendium_pathway_coverage.tsv"))
```
...
saveRDS(both_tech_plier_list, 
        file = file.path(plier_result_dir,"both_technologies_PLIER_list.RDS"))
saveRDS(microarray_plier_list, 
        file = file.path(plier_result_dir, "microarray_PLIER_list.RDS"))
saveRDS(rnaseq_plier_list, 
        file = file.path(plier_result_dir, "rnaseq_PLIER_list.RDS"))
```
...
ggplot2::ggsave(file.path(plot_dir, 
                          "test_compendium_plier_metrics_by_tech.png"),
                height = 8.5, width = 11,
