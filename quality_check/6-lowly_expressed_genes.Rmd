...
```{r}
example_mode <- CalculateMode(seq_mat[, 34])
...
data.frame(x = seq_mat[, another_random_index]) %>%
  ...
  ggplot2::labs(y = "ecdf(x)", x = "expression value (x)") +
  ggplot2::theme_bw() +
  ggplot2::geom_vline(xintercept = CalculateMode(seq_mat[, another_random_index]),
                      lty = "dotted")
```
...
```{r}
seq_col_modes <- apply(seq_mat, 2, CalculateMode)
```
